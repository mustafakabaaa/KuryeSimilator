...
public class DialogueUI : MonoBehaviour
{
    ...
    {  // Kamera kilidini kaldır
        ...
        // 3. İmleç kontrolü (Debug ekleyerek test edin)
        Debug.Log("Closing dialogue - Setting cursor: visible=false, locked");
        Cursor.visible = false;
        Cursor.lockState = CursorLockMode.Locked;

        // 4. Input sistemini resetle (EventSystem çakışmaları için)
    ...
    private void ShowNode(DialogueNode node)
    {
        ...
        {
            // Seçenek yoksa 2 saniye bekleyip diyalogu kapat
    ...
    IEnumerator AutoCloseDialogue(float delay)
    {
        ...
        UnityEngine.EventSystems.EventSystem.current.SetSelectedGameObject(null); // ← Buraya da eklenebilir
    ...
    private void SelectOption(int nextNodeIndex)
    {
        if (nextNodeIndex == -1)
        {
            // 1. SİPARİŞ TAMAMLA (NPC kalır)
            string completedOrderID = DialogueManager.Instance.GetCurrentOrderID();
            if (!string.IsNullOrEmpty(completedOrderID))
            {
                OrderManager.Instance.CompleteOrder(completedOrderID); // Sipariş tamamlandı
            }
            CloseDialogue();
        }
        else if (nextNodeIndex == -2)
        {
            // 2. SİPARİŞİ TAMAMLAMA (NPC kalır, sadece diyalog kapanır)
            CloseDialogue(); // Sipariş tamamlanmaz, NPC yok edilmez
        }
        else
        {
            // 3. Normal diyalog akışı
