...
{
    public GameObject orderUIPanel; // Siparişlerin gösterileceği UI paneli
    public Transform orderContainer; // Siparişlerin ekleneceği container (ScrollView Content)
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("Butonlar inspector'da atanmamış!");
            return;
        }
    ...
    private void OnCancelOrderClicked()
    {
        if (OrderManager.Instance.GetActiveOrders().Count > 0)
        {
            ...
            // Cancel butonunu gizle (artık aktif sipariş yoksa)
    ...
    private void ToggleOrders()
    {
        ...
        // Cancel butonunu sadece aktif sipariş varsa göster
